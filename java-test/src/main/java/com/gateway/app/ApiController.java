...
public class ApiController {
     ...
     */
    @GetMapping("/hostedCheckoutqwer")
    public ModelAndView hostedCheckoutReceipt(HttpSession session) {

        String orderId = (String)session.getAttribute("orderId");
        HostedSession hostedSession = (HostedSession)session.getAttribute("hostedSession");
        ...
        try {
            if (result.equals(ApiResponses.SUCCESS.toString())) {
                ...
                ApiRequest req123 = new ApiRequest();
                req123.setApiOperation("TOKENIZE");
                req123.setSessionId(hostedSession.getId());
//                req123.setSourceType("CARD");

                String requestUrl123 = ApiRequestService.getTokenRequestUrl(ApiProtocol.REST, config);
                String data = ApiRequestService.buildJSONPayload(req123);

                RESTApiClient connection123 = new RESTApiClient();
                String resp123 = connection123.postTransaction(data, requestUrl123, config);
